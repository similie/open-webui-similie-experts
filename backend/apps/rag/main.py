...
@app.post("/web")
def store_web(form_data: StoreWebForm, user=Depends(get_current_user), models=Depends(get_all_models)):
...
@app.get("/scan")
def scan_docs_dir(user=Depends(get_admin_user),models=Depends(get_all_models)):
